...
void put_from_to(int *stack_1, int *stack_2, char c, bool print) {
...
void push(char c, int *stack_a, int *stack_b, bool print) {
  if (c == 'a' && stack_b[0])
    put_from_to(stack_b, stack_a, c, print);
  else if (c == 'b' && stack_a[0])
    put_from_to(stack_a, stack_b, c, print);
}

void rotate(int *stack, char c, bool print) {
  int holder;
  int control;
