...
void initialize_struct(t_struct **data, int ac, char **av) {
  ...
  (*data)->sorted =
      fill_and_sort((*data)->stack_a, &((*data)->totalToBeTransfered));
  (*data)->offset = calculat_offset((*data)->totalToBeTransfered);
  (*data)->total = ac - 1;
  (*data)->midle = (*data)->totalToBeTransfered / 2;
  (*data)->big_val_index = (*data)->totalToBeTransfered - 1;
